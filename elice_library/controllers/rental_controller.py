...
class BookRental(Resource):
    def get(self):
        return make_response(render_template('rental/books_rental.html', rental_list=book_rental_service.get_rental_list_by(g.user.id)))
...
class BookReturn(Resource):
    def get(self):
        ...
        return make_response(render_template('rental/books_return.html', rental_list=not_finished))
...
class RentalBest(Resource):
    def get(self):
        return make_response(render_template('rental/rental_best.html', books=book_service.sort_by_rentals_num()))
